...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const groups = await db.select().from(valuationQuestionGroups).orderBy(asc(valuationQuestionGroups.order));
...
export const actions: Actions = {
	createGroup: async ({ locals, request }) => {
		if (!locals.user || locals.user.username !== ADMIN_EMAIL) return fail(401, { error: 'Unauthorized' });
